...
## Hypercube set up
randSeed = 1 # random number of choice
...
for n in range(samples):
    ...
    res = minimize(cost_function, k0, method='Powell', args = (cG_vec, cI_vec), bounds=bounds) #lägg in constraints här 
...
plt.xlabel("Tid [min]", fontsize=12), plt.ylabel("Glukos koncentration [mM]", fontsize=12)
...
path_fig = path_result_dir + "/plot_glucose.pdf"
...
plt.legend((line4, line3, line2, line1), ("Modell", "Data", "Högsta gräns","Lägsta gräns"))
plt.xlabel("Tid [min]", fontsize=12), plt.ylabel("Insulin koncentration [pM]", fontsize=12)
...
plt.legend()
plt.xlabel("Tid [min]", fontsize=12), 
h = plt.ylabel("Glukos koncentration [mM]", fontsize=12, loc= 'top')
h.set_rotation(0)
...
if not os.path.isdir(path_result_dir):
    os.mkdir(path_result_dir)  # Create a new directory if not existing
path_fig = path_result_dir + "/plot_glukoslevern.pdf"
print("path_fig = {}".format(path_fig))
plt.savefig(path_fig)
...
if not os.path.isdir(path_result_dir):
    os.mkdir(path_result_dir)  # Create a new directory if not existing
path_fig = path_result_dir + "/plot_glukosmuskeln.pdf"
print("path_fig = {}".format(path_fig))
plt.savefig(path_fig)
...
if not os.path.isdir(path_result_dir):
    os.mkdir(path_result_dir)  # Create a new directory if not existing
path_fig = path_result_dir + "/plot_glukos_intag.pdf"
print("path_fig = {}".format(path_fig))
plt.savefig(path_fig)
...
if not os.path.isdir(path_result_dir):
    os.mkdir(path_result_dir)  # Create a new directory if not existing
path_fig = path_result_dir + "/plot_glukagon_i_plasma.pdf"
print("path_fig = {}".format(path_fig))
plt.savefig(path_fig)
...
plt.legend()
plt.xlabel("Tid [min]", fontsize=12) 
plt.ylabel("Glukos koncentration [mM]", fontsize=12)
...
if not os.path.isdir(path_result_dir):
    os.mkdir(path_result_dir)  # Create a new directory if not existing
path_fig = path_result_dir + "/plot_Fettreserver.pdf"
print("path_fig = {}".format(path_fig))
plt.savefig(path_fig)