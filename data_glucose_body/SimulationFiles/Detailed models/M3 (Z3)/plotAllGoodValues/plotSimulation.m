...
firstparam(1)=0;
firstparam(2)=0;
firstparam(3:(length(param)+2))=param;
%try
...
initcondWb(33:44)=icOrigWb(33:44);
...
for i=1:8
    initcondGlu5(:,i)=simData(i).statevalues(1000,:);
    paramGlu5(:,i)=paramP(:,i);
    paramGlu5(2,i)=170;
end
try
    for i=1:8
...
%GLUCOSE UPTAKE 5 mM glucose
for i=1:2
    simGLU5(i)=simData5(i).reactionvalues(end,1);
end
simGLU5(3)=simData5(8).reactionvalues(end,1);
for i=3:7
    simGLU5(i+1)=simData5(i).reactionvalues(end,1);
end
...
%Simulations with 100 nM insulin for 30 min
try
    simDataIR = SBAOsimulate(modelName,timeIR,initcond,names,paramP(:,7),simOptions); 
catch disp('Now the simulation crashed...');
   error = inf;
...
%Simulation with 1.2 nM insulin for 1.5 min (double-step)
try
    simDataDOUBLE1 = SBAOsimulate(modelName,timeDOUBLE(1:40),initcond,names,paramP(:,9),simOptions); 
catch disp('Now the simulation simDataDOUBLE1 crashed...');
   error = inf;
   return 
end
...
%Simulation with 10 nM insulin for 4 min (double-step)
try
    simDataDOUBLE2 = SBAOsimulate(modelName,timeDOUBLE(40:100),initcondDOUBLE,names,paramP(:,6),simOptions); 
catch disp('Now the simulation simDataDOUBLE2 crashed...');
   error = inf;
   return 
end
...
for i=1:61
    simIRS1double(i+40)=simDataDOUBLE2.variablevalues(i,2)/maxIRS1double;
end
...
%Internalized IR
for i = 0:100
    simIntIR(1+i)=simData(6).variablevalues(1+10*i,4)*10;
end
...
figure(7)
semilogx(insulinConcentration2,simGLU5,'b-');
figure(10)
plot(timeGLU,simGLUtime,'b-');
figure(11)
plot(timeIR,simIRtime,'b-');
figure(12)
plot(timeIR,simIRS1time,'b-');
figure(13)
plot(timeDOUBLE,simIRS1double,'b-')
figure(14)
plot(timeIRS1,simIRS1time3,'b-')
figure(15)
plot(intTime,simIntIR,'b-')
